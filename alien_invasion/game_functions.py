...
def check_keydown_events(event,ship,screen,ai_settings,bullets):
    ...
    if event.key == pygame.K_SPACE and len(bullets) < ai_settings.bullet_allowde:
...
def check_event(ship,bullets,ai_settings,screen,stats,play_button,aliens,sb):
...
def update_alien(ai_settings,aliens,ship,bullets,screen,stats,sb):
